...
class demoCallbacksClient:
    ...
    async def on_packet(
        self, message
    ):  # Called when any packet is received, regardless of packet command.
    ...
    ):  # Called when a packet is received with the direct command.
        print("on_direct fired!")

    async def on_version(
        self, version: str
    ):  # Called when a packet is received with the server_version command.
        print("on_version fired!")

    async def on_motd(
        self, motd: str
    ):  # Called when a packet is received with the motd command.
        print("on_motd fired!")

    async def on_ip(
        self, ip: str
    ):  # Called when a packet is received with the client_ip command.
        print("on_ip fired!")
    ...
    async def on_gmsg(
        self, message: any
    ):  # Called when a packet is received with the gmsg command.
        print("on_gmsg fired!")

    async def on_gvar(
        self, var_name: str, var_value: any
    ):  # Called when a packet is received with the gvar command.
        print("on_gvar fired!")
    ...
    async def on_ping(
        self, value: str, origin: any
    ):  # Called when the client is being pinged by another client (It will automatically respond to the ping, this is just used for diagnostics).
        print("on_ping fired!")
