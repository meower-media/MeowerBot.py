...
class MeowerAPI:
    ...
    async def get_post(self, uuid: generic.UUID):
        return post_resp(await self.client.get(f"/posts", params={"id": uuid}))


    async def update_post(self, uuid: generic.UUID,  content: str) -> Post:
        return post_resp(await self.client.patch(f"/posts", params={"id": uuid}, json={"content": content}))

    async def delete_post(self, uuid: generic.UUID) -> Post:
        return post_resp(await self.client.patch(f"/posts", params={"id": uuid}))
