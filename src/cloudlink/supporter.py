...
class supporter:
    ...
    async def _sendCode(
        ...
        message = {"cmd": "statuscode", "code": self.codes[code]}
    ...
    def callback(self, callback_id, function):
        ...
        if callable(callback_id):
            if hasattr(self.cloudlink, callback_id.__name__):
                self.log(
                    f"Creating callback for {callback_id.__name__} to function {function.__name__}..."
                )
        ...
        for functionEntry in functions:
            ...
            if not ("__" in functionEntry) and (hasattr(self.cloudlink, functionEntry)):
                ...
                # Something else
                    ...
                    self.log(
                        f"Attempted to disable command {functionEntry} but either the command was already unloaded, it was not valid, or was not loaded beforehand."
                    )
                    ...
                    if not hasattr(self.cloudlink, str(classEntry)):
                        if (custom != None) and (classEntry in custom):
                            setattr(
                                self.cloudlink,
                                str(classEntry),
                                classEntry(self.cloudlink, custom[classEntry]),
                            )
                        else:
                            setattr(
                                self.cloudlink,
                                str(classEntry),
                                classEntry(self.cloudlink),
                            )

                    for functionEntry in classFunctions:
                        # Check if a function within class is a private function
                        try:
                            if (not ("__" in functionEntry)) and (
                                not (hasattr(self.cloudlink, functionEntry))
                            ):
                                # Check if a function is marked as ignore
                                shouldAdd = True
                                if hasattr(
                                    getattr(self.cloudlink, str(classEntry)),
                                    "importer_ignore_functions",
                                ):
                                    if (
                                        type(
                                            getattr(
                                                self.cloudlink, str(classEntry)
                                            ).importer_ignore_functions
                                        )
                                        == list
                                    ):
                                ...
                                if shouldAdd:
                                    ...
                                    self.cloudlink.customCommands.append(
                                        str(functionEntry)
                                    )
                        except:
                            self.log(
                                f"An exception has occurred whilst loading a custom command: {self.full_stack()}"
                            )
                else:
                    raise TypeError(
                        f'Attempted to load "{str(classEntry)}", which is not a class!'
                    )
                ...
                try:
                    ...
                    if hasattr(handlerObject, "importer_ignore_functions"):
                        if type(handlerObject.importer_ignore_functions) == list:
                            if (
                                str(functionEntry)
                                in handlerObject.importer_ignore_functions
                            ):
                                shouldAdd = False
                    ...
                    if shouldAdd:
                        ...
                        setattr(
                            self.cloudlink,
                            str(functionEntry),
                            getattr(handlerObject, str(functionEntry)),
                        )
                except:
                    self.log(
                        f"An exception has occurred whilst loading a built-in command: {self.full_stack()}"
                    )
