...
class serverRootHandlers:
    ...
    async def on_connect(self, client):
        ...
        if self.cloudlink.rejectClientMode:
            ...
            self.supporter.log(
                f"Client {client.id} ({client.full_ip}) was blocked from connecting."
            )
            ...
            await self.cloudlink.sendPacket(
                client,
                {"cmd": "client_ip", "val": str(client.full_ip)},
                ignore_rooms=True,
            )
            ...
            await self.cloudlink.sendPacket(
                client,
                {"cmd": "server_version", "val": str(self.cloudlink.version)},
                ignore_rooms=True,
            )
            ...
            await self.cloudlink.sendPacket(
                client,
                {"cmd": "gmsg", "val": self.cloudlink.global_msg},
                ignore_rooms=True,
            )
            ...
            if self.cloudlink.motd_enable:
                await self.cloudlink.sendPacket(
                    client,
                    {"cmd": "motd", "val": self.cloudlink.motd_msg},
                    ignore_rooms=True,
                )
    ...
    async def on_close(self, client, code, reason):
        ...
        self.supporter.log(
            f'Client {client.id} ({client.full_ip}) disconnected with code {code} and reason "{reason}"'
        )
        ...
        for room in tmp_roomData:
            ...
            pages, size, ulist = self.supporter.paginate_ulist(
                self.cloudlink.getUsernames(room)
            )
            await self.cloudlink.sendPacket(
                clist,
                {"cmd": "ulist", "pages": pages, "size": size, "val": ulist},
                    ...
                    if "rooms" in message:
                        ...
                        for room in tmp_room_id:
                            if not room in client.rooms:
                                self.supporter.log(
                                    f"Client {client.id} ({client.full_ip}) attempted to access room {room}, but was blocked!"
                                )
                                room_id.remove(room)

                    # Check if the command is a built-in Cloudlink command
                    if (message["cmd"] in self.cloudlink.builtInCommands) and not (
                        message["cmd"] == "direct"
                    ):
                        await getattr(self.cloudlink, str(message["cmd"]))(
                            client, message, listener_detected, listener_id, room_id
                        )
                        # Fire callbacks
                        if self.on_packet in self.cloudlink.usercallbacks:
                            if self.cloudlink.usercallbacks[self.on_packet] != None:
                                await self.cloudlink.usercallbacks[self.on_packet](
                        ...
                        if message["cmd"] in self.cloudlink.customCommands:
                            ...
                            if self.supporter.isPacketSane(message["val"]):
                                if type(message["val"]) == dict:
                                    if (
                                        message["val"]["cmd"]
                                        in self.cloudlink.customCommands
                                    ):
                        ...
                        if isValid:
                            if isLegacy:
                                self.supporter.log(
                                    f"Client {client.id} ({client.full_ip}) sent legacy custom command \"{message['val']['cmd']}\""
                                )
                                await getattr(
                                    self.cloudlink, str(message["val"]["cmd"])
                                )(
                                    client,
                                    message["val"],
                                    listener_detected,
                                ...
                                if isCustom:
                                    self.supporter.log(
                                        f"Client {client.id} ({client.full_ip}) sent custom command \"{message['cmd']}\""
                                    )
                                    await getattr(self.cloudlink, str(message["cmd"]))(
                                        client,
                                        message,
                                    ...
                                    await getattr(self.cloudlink, "direct")(
                                        client,
                                        message,
                                        listener_detected,
                                        listener_id,
                            ...
                            if message["cmd"] in self.cloudlink.disabledCommands:
                                self.supporter.log(
                                    f"Client {client.id} ({client.full_ip}) sent custom command \"{message['cmd']}\", but the command is disabled."
                                )
                                await self.cloudlink.sendCode(
                                    client, "Disabled", listener_detected, listener_id
                                )
                            else:
                                self.supporter.log(
                                    f"Client {client.id} ({client.full_ip}) sent custom command \"{message['cmd']}\", but the command is invalid or it was not loaded."
                                )
                                await self.cloudlink.sendCode(
                                    client, "Invalid", listener_detected, listener_id
                                )

                else:
                    self.supporter.log(
                        f'Packet "{message}" is invalid, incomplete, or malformed!'
                    )
                    await self.cloudlink.sendCode(client, "Syntax")
            except:
                self.supporter.log(
                    f"An exception has occurred. {self.supporter.full_stack()}"
                )
