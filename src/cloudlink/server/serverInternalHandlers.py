...
class serverInternalHandlers:
    ...
    async def ulist(self, client, message, listener_detected, listener_id, room_id):
        if not type(message["val"]) == int:
            await self.cloudlink.sendCode(
                client, "DataType", listener_detected, listener_id
            )
            return

        for room in client.rooms:
            pages, size, ulist = self.supporter.paginate_ulist(
                self.cloudlink.getUsernames(room), page_select=message["val"]
            )
            await self.cloudlink.sendPacket(
                client,
                {"cmd": "ulist", "pages": pages, "size": size, "val": ulist},
                rooms=room,
            )
    ...
    async def statuscode(
        ...
        for key in ["val", "id"]:
            if not key in message:
                await self.cloudlink.sendCode(
                    client, "Syntax", listener_detected, listener_id
                )
                return
        ...
        if type(message["id"]) == list:
            ...
            if not (len(rx_client) == 0):
                ...
                self.supporter.log(
                    f"Client {client.id} ({client.full_ip}) is sending a status code to other clients!"
                )
                await self.cloudlink.sendPacket(
                    rx_client,
                    {
                        "cmd": "statuscode",
                        "code": message["val"],
                        "origin": self.cloudlink.getUserObjectFromClientObj(client),
                    },
                )

                # Tell the client that all status codes were successfully sent
                await self.cloudlink.sendCode(
                    client, "OK", listener_detected, listener_id
                )
            else:
                # Tell the client that the server failed to find a client with those IDs
                await self.cloudlink.sendCode(
                    client, "IDNotFound", listener_detected, listener_id
                )
        else:
            ...
            if rx_client == LookupError:
                ...
                await self.cloudlink.sendCode(
                    client, "IDNotSpecific", listener_detected, listener_id
                )
                return
            ...
            self.supporter.log(
                f"Client {client.id} ({client.full_ip}) is sending a status code to {rx_client.id} ({rx_client.full_ip})!"
            )
            await self.cloudlink.sendPacket(
                rx_client,
                {
    ...
    async def ping(self, client, message, listener_detected, listener_id, room_id):
        ...
        if not "id" in message:
            self.supporter.log(
                f"Client {client.id} ({client.full_ip}) pinged the server!"
            )
            await self.cloudlink.sendCode(client, "OK", listener_detected, listener_id)
            return
        ...
        if not client.username_set:
            await self.cloudlink.sendCode(
                client, "IDRequired", listener_detected, listener_id
            )
            return
        ...
        if type(message["id"]) == list:
            ...
            if not (len(rx_client) == 0):
                # Send the message to all recipient IDs
                self.supporter.log(
                    f"Client {client.id} ({client.full_ip}) is pinging other clients!"
                )
                await self.cloudlink.sendPacket(
                    rx_client,
                    {
            ...
            self.supporter.log(
                f"Client {client.id} ({client.full_ip}) is sending a ping to {rx_client.id} ({rx_client.full_ip})!"
            )
    ...
    async def link(self, client, message, listener_detected, listener_id, room_id):
        self.supporter.log(
            f"Client {client.id} ({client.full_ip}) is linking to room(s): {message['val']}"
        )
        ...
        if type(message["val"]) in [list, str]:
            ...
            if ("default" in old_rooms) and (not "default" in message["val"]):
                pages, size, ulist = self.supporter.paginate_ulist(
                    self.cloudlink.getUsernames("default")
                )
                await self.cloudlink.sendPacket(
                    self.cloudlink.getAllUsersInRoom("default"),
                    {"cmd": "ulist", "pages": pages, "size": size, "val": ulist},
                )
            ...
            for room in new_rooms:
                ...
                await self.cloudlink.sendPacket(
                    self.cloudlink.getAllUsersInRoom(room),
                    {"cmd": "ulist", "val": self.cloudlink.getUsernames(room)},
                    rooms=room,
                )
    ...
    async def unlink(self, client, message, listener_detected, listener_id, room_id):
        self.supporter.log(
            f"Client {client.id} ({client.full_ip}) unlinking from all rooms"
        )
        ...
        if client.is_linked:
            ...
            pages, size, ulist = self.supporter.paginate_ulist(
                self.cloudlink.getUsernames(old_rooms)
            )
            await self.cloudlink.sendPacket(
                self.cloudlink.getAllUsersInManyRooms(old_rooms),
                {"cmd": "ulist", "pages": pages, "size": size, "val": ulist},
                rooms=old_rooms,
            )
        else:
            self.supporter.log(
                f"Client {client.id} ({client.full_ip}) was already unlinked!"
            )
            ...
            await self.cloudlink.sendPacket(
                client, {"cmd": "ulist", "val": self.cloudlink.getUsernames()}
            )
    ...
    async def direct(self, client, message, listener_detected, listener_id, room_id):
        ...
        if not "id" in message:
            if self.direct in self.cloudlink.usercallbacks:
                self.supporter.log(
                    f'Client {client.id} ({client.full_ip}) sent direct data: "{message}"'
                )
                # Fire callbacks
                if self.direct in self.cloudlink.usercallbacks:
                    if self.cloudlink.usercallbacks[self.direct] != None:
                        await self.cloudlink.usercallbacks[self.direct](
                            message=message,
                            origin=self.cloudlink.getUserObject(client),
        ...
        if type(message["id"]) == list:
            ...
            if not (len(rx_client) == 0):
                # Send the message to all recipient IDs
                self.supporter.log(
                    f"Client {client.id} ({client.full_ip}) is sending direct data to various clients!"
                )
                await self.cloudlink.sendPacket(
                    rx_client,
                    {
            ...
            self.supporter.log(
                f"Client {client.id} ({client.full_ip}) is sending direct data to {rx_client.id} ({rx_client.full_ip})!"
            )
            # Send the ping to the recipient ID
            await self.cloudlink.sendPacket(
                rx_client,
    ...
    async def gmsg(self, client, message, listener_detected, listener_id, room_id):
        self.supporter.log(
            f"Client {client.id} ({client.full_ip}) sent global message with data \"{message['val']}\""
        )
        msg = {"cmd": "gmsg", "val": message["val"]}
        ...
        await self.cloudlink.sendPacket(
            client, msg, listener_detected, listener_id, room_id
        )
    ...
    async def gvar(self, client, message, listener_detected, listener_id, room_id):
        ...
        self.supporter.log(
            f"Client {client.id} ({client.full_ip}) sent global variable with data \"{message['val']}\""
        )

        msg = {"cmd": "gvar", "val": message["val"], "name": message["name"]}
        ...
        if self.gvar in self.cloudlink.usercallbacks:
            if self.cloudlink.usercallbacks[self.gvar] != None:
                await self.cloudlink.usercallbacks[self.gvar](
                    var_value=message["val"], var_name=message["name"]
                )
    ...
    async def pvar(self, client, message, listener_detected, listener_id, room_id):
        ...
        if type(message["id"]) == list:
            ...
            if not (len(rx_client) == 0):
                # Send the message to all recipient IDs
                self.supporter.log(
                    f"Client {client.id} ({client.full_ip}) sent private variable data \"{message['val']}\" going to various clients!"
                )
                await self.cloudlink.sendPacket(
                    rx_client,
                    {
            ...
            self.supporter.log(
                f"Client {client.id} ({client.full_ip}) sent private variable data \"{message['val']}\" going to {rx_client.id} ({rx_client.full_ip})!"
            )
            await self.cloudlink.sendPacket(
                rx_client,
                {
        ...
        if self.pvar in self.cloudlink.usercallbacks:
            if self.cloudlink.usercallbacks[self.pvar] != None:
                await self.cloudlink.usercallbacks[self.pvar](
                    var_value=message["val"],
    ...
    async def pmsg(self, client, message, listener_detected, listener_id, room_id):
        ...
        if type(message["id"]) == list:
            ...
            if not (len(rx_client) == 0):
                # Send the message to all recipient IDs
                self.supporter.log(
                    f"Client {client.id} ({client.full_ip}) sent private messsage data \"{message['val']}\" going to various clients!"
                )
                await self.cloudlink.sendPacket(
                    rx_client,
                    {
            ...
            # Send the message to the recipient ID
            self.supporter.log(
                f"Client {client.id} ({client.full_ip}) sent private message data \"{message['val']}\" going to {rx_client.id} ({rx_client.full_ip})!"
            )
            await self.cloudlink.sendPacket(
                rx_client,
                {
        ...
        if self.pmsg in self.cloudlink.usercallbacks:
            if self.cloudlink.usercallbacks[self.pmsg] != None:
                await self.cloudlink.usercallbacks[self.pmsg](
                    value=message, origin=self.cloudlink.getUserObject(client)
                )
    ...
    async def setid(self, client, message, listener_detected, listener_id, room_id):
        ...
        if client.username_set:
            self.supporter.log(
                f"Client {client.id} ({client.full_ip}) specified username \"{message['val']}\", but username was already set to \"{client.friendly_username}\""
            )
            await self.cloudlink.sendCode(
                client, "IDSet", listener_detected, listener_id
            )
            return
        ...
        if not type(message["val"]) == str:
            self.supporter.log(
                f"Client {client.id} ({client.full_ip}) specified username \"{message['val']}\", but username is not the correct datatype!"
            )
            await self.cloudlink.sendCode(
                client, "DataType", listener_detected, listener_id
            )
        ...
        if not len(message["val"]) in range(1, 21):
            self.supporter.log(
                f"Client {client.id} ({client.full_ip}) specified username \"{message['val']}\", but username is not within 1-20 characters!"
            )
            await self.cloudlink.sendCode(
                client, "Refused", listener_detected, listener_id
            )
            return

        self.supporter.log(
            f"Client {client.id} ({client.full_ip}) specified username \"{message['val']}\""
        )
        client.friendly_username = str(message["val"])
        client.username_set = True
        # Report to the client that the username was accepted
        msg = {"username": client.friendly_username, "id": client.id}
