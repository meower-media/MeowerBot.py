class clientRootHandlers:
    ...
    async def on_close(self, close_status_code, close_msg):
        if self.cloudlink.linkStatus == 1:
            ...
            self.supporter.log(
                f'Client failed to connect! Disconnected with status code {close_status_code} and message "{close_msg}"'
            )
        elif self.cloudlink.linkStatus == 2:
            self.cloudlink.linkStatus = 4
            self.cloudlink.connectionLost = True
            self.supporter.log(
                f'Client lost connection! Disconnected with status code {close_status_code} and message "{close_msg}"'
            )

        # Fire callbacks
        if self.on_close in self.cloudlink.usercallbacks:
            if self.cloudlink.usercallbacks[self.on_close] != None:
                await self.cloudlink.usercallbacks[self.on_close](
                ...
                if isPacketSane:
                    ...
                    if (message["cmd"] in self.cloudlink.builtInCommands) and not (
                        ...
                        if isValid:
                            if isLegacy:
                                self.supporter.log(
                                    f"Client received legacy custom command \"{message['val']['cmd']}\""
                                )
                                await getattr(
                                ...
                                if isCustom:
                                    self.supporter.log(
                                        f"Client received custom command \"{message['cmd']}\""
                                    )
                            ...
                            if message["cmd"] in self.cloudlink.disabledCommands:
                                self.supporter.log(
                                    f"Client received command \"{message['cmd']}\", but the command is disabled."
                                )
                            else:
                                self.supporter.log(
                                    f"Client received command \"{message['cmd']}\", but the command is invalid or it was not loaded."
                                )
                else:
                    self.supporter.log(
