class clientInternalHandlers:
    ...
    async def statuscode(self, message):
        if "listener" in message:
            self.supporter.log(
                f"Client received status code for handler {message['listener']}: {message['code']}"
            )

            # Check if the username was set
            if (message["listener"] == "username_set") and (
                message["code"] == self.supporter.codes["OK"]
            ):
                self.cloudlink.myClientObject = message["val"]
                self.supporter.log(
                    f"Client received it's user object: {self.cloudlink.myClientObject}"
                )

            # Check if a ping was successful
            if (message["listener"] == "ping_handler") and (
                message["code"] == self.supporter.codes["OK"]
            ):
        ...
        if self.statuscode in self.cloudlink.usercallbacks:
            if self.cloudlink.usercallbacks[self.statuscode] != None:
                await self.cloudlink.usercallbacks[self.statuscode](
    ...
    async def gmsg(self, message):
        self.supporter.log(
            f"Client received global message with data \"{message['val']}\""
        )
    ...
    async def gvar(self, message):
        self.supporter.log(
            f"Client received global variable with data \"{message['val']}\""
        )
        ...
        if self.gvar in self.cloudlink.usercallbacks:
            if self.cloudlink.usercallbacks[self.gvar] != None:
                await self.cloudlink.usercallbacks[self.gvar](
                    var_name=message["name"], var_value=message["val"]
                )

    # Private cloud variables
    async def pvar(self, message):
        self.supporter.log(
            f"Client received private message with data \"{message['val']}\""
        )
        ...
        if self.pmsg in self.cloudlink.usercallbacks:
            if self.cloudlink.usercallbacks[self.pmsg] != None:
                await self.cloudlink.usercallbacks[self.pmsg](
                    value=message["val"], origin=message["origin"]
                )
    ...
    async def ping(self, message):
        ...
        await self.cloudlink.sendPacket(
            {
                "cmd": "statuscode",
                "val": self.supporter.codes["OK"],
                "id": message["origin"],
                "listener": "ping_handler",
            }
        )
        ...
        if self.ping in self.cloudlink.usercallbacks:
            if self.cloudlink.usercallbacks[self.ping] != None:
                await self.cloudlink.usercallbacks[self.ping](
                    value=message["val"], origin=message["origin"]
                )
