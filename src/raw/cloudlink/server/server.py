...
class server:
    ...
    def __init__(self, parentCl, enable_logs=True):
        ...
        self.rejectClientMode = (
            False  # Set to true to reject future clients until false
        )
        ...
        if type(username) == str:
            ...
            if client in all_clients:
                if not client.username_set:
                    self.log(
                        f'Setting client {client.id} ({client.full_ip}) username to "{username}"...'
                    )
            ...
            if client in all_clients:
                ...
                if type(rooms) in [list, str]:
                    if type(rooms) == str:
                        ...
                        rooms = set(rooms)
                self.log(
                    f"Linking client {client.id} ({client.full_ip}) to rooms {list(rooms)}..."
                )
    ...
    async def __handler__(self, websocket):
        ...
        # Shutdown the connection
            ...
            if not websocket.rejected:
                await self.serverRootHandlers.on_close(
                    websocket, websocket.close_code, websocket.close_reason
                )
